...
export default async function DashboardPage() {
  const [data, connection] = await Promise.all([getDashboard(), readConnectionState()]);
  ...
  return <main className="p-6 md:p-12"><LiveRefresh /><div className="mx-auto max-w-6xl">
    <header className="flex flex-wrap items-start justify-between gap-5">
      ...
      <Link href="/nova-campanha" className="rounded-lg bg-emerald-600 px-5 py-3 font-semibold text-white">Nova campanha</Link>
    </header>
    <div role="status" className="mt-6 flex flex-wrap items-center gap-3 text-sm">
      ...
      <Link href="/configuracoes" className="text-slate-500 underline">Ver conexão</Link>
      {!connected && <span className="text-slate-500">Envios reais aguardam a conexão.</span>}
    </div>
    {!data && <p role="alert" className="mt-8 rounded-lg bg-amber-50 p-4 text-amber-800">Não foi possível consultar os dados. Verifique se a API e o banco estão funcionando.</p>}
    ...
    <div className="grid gap-12 border-t border-slate-200 pt-8 lg:grid-cols-2">
      ...
      <section><h2 className="text-xl font-bold">Atividade recente</h2><p className="mt-2 text-xs text-slate-500">Últimos resultados reais registrados. Falhas usam a data de atualização do registro, não um log completo de tentativas.</p>
        {data?.recentActivity.length ? <ul className="mt-4 divide-y divide-slate-200">{data.recentActivity.map(event => <li key={event.id} className="py-4"><time dateTime={event.at} className="text-xs text-slate-500">{format(event.at)} · Brasília</time><p className={event.status === 'FAILED' ? 'mt-1 text-red-700' : 'mt-1 font-medium'}>{event.status === 'FAILED' ? `Falha registrada para ${event.group.name}` : `${event.group.name} enviado ✓`}</p>{event.campaign.deletedAt ? <p className="mt-1 text-xs text-slate-500">{event.campaign.name} · campanha excluída</p> : <Link href={`/campanhas/${event.campaignId}`} className="mt-1 inline-block text-xs text-slate-500 underline">{event.campaign.name}</Link>}</li>)}</ul> : <p className="mt-5 text-sm text-slate-500">{data ? 'Nenhum resultado de envio real registrado.' : 'Atividade indisponível.'}</p>}
      </section>
    </div>
  </div></main>;
}
