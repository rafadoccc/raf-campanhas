...
export function CampaignActions({ id, status, provider = 'simulator', groupCount = 0, intervalSeconds = 180, mode = 'SCHEDULED', connectionState = 'unavailable' }: { id: string; status: string; provider?: string; groupCount?: number; intervalSeconds?: number; mode?: string; connectionState?: string }) {
  ...
  async function update(nextStatus: 'ACTIVE' | 'PAUSED' | 'CANCELLED') {
    ...
    try {
      const response = await fetch(`${apiUrl}/campaigns/${id}/status`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: nextStatus, provider: selected, consent: selected === 'baileys' }) });
  ...
  async function remove() {
    ...
    setBusy(true); setError('');
    try { const r = await fetch(`${apiUrl}/campaigns/${id}`, { method: 'DELETE' }); const data = await r.json(); if (!r.ok) throw new Error(data.error); router.push('/campanhas'); router.refresh(); }
  ...
  return <div className="mt-4 flex flex-wrap items-center gap-2">
    ...
    {status === 'DRAFT' && <a href={`/campanhas/${id}/editar`} className="rounded border px-3 py-2 text-emerald-700">Editar</a>}
